...
void startup_task(void *pvParameters) {
  while (true) {
    // 等待启动信号量
    if (xSemaphoreTake(startup_xBinarySemaphore, portMAX_DELAY) == pdTRUE) {
...
void setup() {
  ...
  lv_port_indev_init(); // 初始化和绑定触摸接口
   ...
   setup_scr_startup_page(&guider_ui); // gui_guider 为每一个页面生成的，这里是名字为 xxx 的页面
   setup_scr_initial_page(&guider_ui); // gui_guider 为每一个页面生成的，这里是名字为 xxx 的页面
  ...
  if (current_control_queue == NULL) {
   ...
   xTaskCreatePinnedToCore(update_gui_task,
    ...
    lv_scr_load(guider_ui.initial_page); // 加载初始页面
  //  lv_scr_load(guider_ui.startup_page); // 加载启动页面
  ...
  startup_xBinarySemaphore = xSemaphoreCreateBinary();
  if (button_xBinarySemaphore != NULL && startup_xBinarySemaphore != NULL) {
    // Core 1 运行按键处理任务
    ...
    xTaskCreatePinnedToCore(startup_task,
              "startup_task",
              1024*4,
              NULL,
              1, // 低优先级
