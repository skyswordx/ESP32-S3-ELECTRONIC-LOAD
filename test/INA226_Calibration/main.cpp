...
void setup()
{
  ...
  /* USER SET VALUES */
  const float CURRENT_OFFET_mA = -0.525;
  const float BUS_V_DMM = 6.013; // DMM 测量的电压
  const float BUS_V_SERIAL= 6.109; // 串口测量的电压


  float shunt = 0.010;     // R010           /* shunt (Shunt Resistance in Ohms). Lower shunt gives higher accuracy but lower current measurement range. Recommended value 0.020 Ohm. Min 0.001 Ohm */
  ...
  float current_zero_offset_mA = CURRENT_OFFET_mA;         /* current_zero_offset_mA (Current Zero Offset in milli Amperes, default = 0) */
  uint16_t bus_V_scaling_e4 = 10000 / BUS_V_SERIAL * BUS_V_DMM;        /* bus_V_scaling_e4 (Bus Voltage Scaling Factor, default = 10000) */
